...
description: "A closer look at the people behind petitions"
...
```{r qs_petrs_histogram_petrs_per_petition}
#| fig-alt: "histogram showing numbers of named petitioners per petition in the Power of Petitioning Quarter Sessions collections. The largest group of petitioners (about 1200) has only one petitioner."
...
```{r qs_petrs_histogram_subscribers_per_petition}
#| fig-alt: "histogram of subscribers to petitions in the Power of Petitioning Quarter Sessions collections; as with petitioners the largest single group have just one subscriber, but it is a much wider distribution with significant numbers of petitions having up to 25 subscribers."
...
```{r  qs_petrs_scatter_trend_solo_petitions_decade}
#| fig-alt: "scatter plot with trend line of percentage of petitions with a single petitioner in the Power of Petitioning Quarter Sessions collections, by decade between late 16th and late 18th centuries. There is probably a slight overall increase over the period, but it fluctuates a good deal."
...
```{r qs_petrs_waffle_gender_petitioners}
#| fig-alt: "waffle chart of petitioner gender in the Power of Petitioning Quarter Sessions collections. 1356 petitioners are male, 442 female."
...
```{r qs_petrs_waffle_gender_subscribers}
#| fig-alt: "waffle chart of petition subscribers gender in the Power of Petitioning Quarter Sessions collections: 3313 male, 71 female."
...
```{r qs_petrs_waffle_gender_petitioners_pet_type}
#| fig-alt: "waffle chart of petitioner gender in the Power of Petitioning Quarter Sessions collections, comparing single vs group petitions. About 77% of female petitioners are solo, and 66% of male petitioners."

# TILr viz geom_waffle: for fill legend correct ordering. in scale_fill* use guide = guide_legend(reverse = TRUE)
qs_petitioners |>
...
```{r qs_petrs_histogram_gender_petitioners_per_petition}
#| fig-alt: "histograms of petitioners per petition in the Power of Petitioning Quarter Sessions collections, comparing all female, all male and mixed gender petitions."
...
```{r qs_petrs_waffle_gender_petitioners_county}
#| fig-alt: "waffle charts of petitioner gender, breakdown by county."

# thread https://twitter.com/sharon_howard/status/1222197505380749312  [2020/01/28]
# Who likes waffle charts?

# viz geom_waffle: for fill legend correct ordering. in scale_fill* use guide = guide_legend(reverse = TRUE)
qs_petitioners |>
  filter(gender %in% c("male", "female")) |>
  count(county, gender) |> 
  ggplot(aes(fill=gender, values=n)) +
...
```{r qs_petrs_waffle_gender_petition_topic}
#| fig-alt: "waffle charts of petitioner gender in the Power of Petitioning Quarter Sessions collections, breakdown by petition topics."
...
```{r qs_petrs_waffle_petitioners_signatures}
#| fig-alt: "waffle chart of petitioner signatures in the Power of Petitioning Quarter Sessions collections. The majority did not sign their petitions and very few who did sign used a mark."
...
```{r qs_petrs_waffle_subscribers_signatures}
#| fig-alt: "waffle chart of subscriber signatures in the Power of Petitioning Quarter Sessions collections. "
...
```{r qs_petrs_waffle_petitioners_signatures_county}
#| fig-alt: "waffle charts of petitioner signatures in the Power of Petitioning Quarter Sessions collections, comparing by county."
...
```{r qs_petrs_waffle_petitioners_signatures_gender}
#| fig-alt: "waffle charts of petitioner signatures in the Power of Petitioning Quarter Sessions collections, comparing male and female petitioners. Only about 5% of female petitioners signed their petitions, compared to 21% of men."
...
#| fig-alt: "lollip chart of percentage of petitioners in the Power of Petitioning Quarter Sessions collections who signed petitions, comparing by petition topic. Only debtors and dissenters signed more than 50% of the time; in the topics related to poor relief less than 10% of petitioners did so."
...
```{r qs_petrs_scatter_petitions_with_subscribers_decade }
#| fig-alt: "scatter chart of the percentage of petitions with subscribers in the Power of Petitioning Quarter Sessions collections, by decade between late 16th and late 18th century."
