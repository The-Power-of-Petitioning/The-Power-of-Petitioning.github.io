...
```{r proportional-bar-detailed-responses-to-cheshire-qs-petitions-by-petition-gender-type-topic, fig.alt='Proportional stacked bar chart of detailed response categories to petitions broken down by topic and petition gender (excluding mixed gender).'}
ches_petition_responses_fct %>%
  filter(pet_gender %in% c("m" , "f")) %>%
  count(topic, response, pet_gender) %>%
  # inner_join(
  #  ches_petition_responses_fct  %>%
  # count(topic, response_s, name="nrs") %>%
  # group_by(topic) %>%
  ...
  theme(legend.position = "left") +
  #theme(axis.text.x =  element_text(angle=45, vjust=0.5, size=9)) +
  labs(y="% of petitions", x=NULL, fill="responses", title="Responses by topic and gender (m/f only)")
