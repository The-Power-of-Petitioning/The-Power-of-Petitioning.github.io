...
# but i think the only thing this adds is the response_details which you don't use again? can recreate has_response.don't actually seem to use it! or details. ok...
# the full ches petitions metadata with all the crap
# need to drop unphotographed
# qs_ches_full_sqlite <-
...
#   select(reference, has_response, response_cat, response_details, year, decade, topic, petition_type, pet_gender, subscribers) %>%
...
```{r}
#petitions_responses %>% count(county)
```
...
I'll exclude the small uncertain group from analysis (Cheshire has only 20 and the others 32, some of which may not be responses), so I'm looking at 593 Cheshire petitions and 1083 others.
...
```{r waffle-detailed-responses-to-cheshire-qs-petitions, fig.alt='Waffle chart of detailed response categories for Quarter Sessions petitions.'}
...
petitions_responses %>%
  ...
  labs(fill="type", title="Breakdown of responses to QS petitions", caption="1 square=1% of petitions")
```
...
```{r waffle-less-detailed-responses-to-cheshire-qs-petitions, fig.alt='Waffle chart of detailed response categories for Quarter Sessions petitions, combining all negative responses into a single category.'}
petitions_responses  %>%  
...
#   theme(legend.position = "left") +
#   #theme(axis.text.x =  element_text(angle=45, vjust=0.5, size=9)) +
#   labs(y="% of petitions", x=NULL, fill="responses")  #, title="Responses to other QS Petitions by topic 1573-1798") 
```
...
ches_petition_responses_fct %>%
  ...
  labs(y="% responses positive", x="number of petitions in topic", title="Positive response clusters by size of topic (Cheshire QS only)")
```
...
qs_level1_petitions_responses_fct %>%
  ...
  scale_fill_brewer(palette = "RdYlBu") +
  facet_wrap(~petition_type_s) +
  labs(title="Responses to other QS petitions by petition type") 
```
...
[Updated] Breakdown by topic as well as gender (Cheshire only; excluding mixed gender).
...
# ches_petition_responses_fct %>%
#   filter(petition_type_s !="collective")
#with(ches_petition_responses_fct, table(pet_gender, response))
# mosaic all grey!
```

```{r}
#vcd::mosaic(with(cpr_gender_resp, table(pet_gender, topic)), shade=TRUE, legend=FALSE)
```


```{r}
#vcd::mosaic(with(ches_petition_responses_fct, table(topic, response)), shade=TRUE, legend=FALSE)
```


```{r}
#vcd::mosaic(with(ches_petition_responses_fct, table(petition_type_s, response)), shade=TRUE)
```
...
```{r}
# structable(c1 + c2 ~ r1, df) ####
# specify col ~ row variables
# cpr_fm <-
#   ches_petition_responses_fct %>%
#   filter(pet_gender %in% c("m", "f"))

# mosaic(
#   structable(topic ~ response + pet_gender, cpr_fm), 
#   shade = TRUE, legend=FALSE,
#   #spacing = spacing_increase(start = unit(0.2, "lines"), rate = 2) , 
#     labeling_args = list(
#       #clip = c(response=TRUE, topic=FALSE, pet_gender=FALSE),
#       abbreviate = c(response=6, topic=8),
#       set_varnames=c(topic="", response="", pet_gender=""),
#       rot_labels = c(90,0,0,0),
#       offset_labels = c(0.7, 0, 0,0.6),
#       #pos_varnames = c("left", "left", "left"),
